...
app.use(async (req, res, next) => {
  ...
  try {
    if (req.url == "/ping") {
      ...
      req.url.indexOf(dataSetting["name-folder-sitemap"]) > 0 &&
...
<urlset xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:image="http://www.google.com/schemas/sitemap-image/1.1" xsi:schemaLocation="http://www.sitemaps.org/schemas/sitemap/0.9 http://www.sitemaps.org/schemas/sitemap/0.9/sitemap.xsd http://www.google.com/schemas/sitemap-image/1.1 http://www.google.com/schemas/sitemap-image/1.1/sitemap-image.xsd" xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">\n`
            ...
            listUrlSitemap.forEach(function (a) {
              ...
              res.write(` <url>\n`);
              res.write(
                `   <loc>` + originUrl + "/host" + "-" + dataUrl + `</loc>\n`
              );
              res.write(
...
<!-- XML Sitemap generated by NodeJs -->`);
      ...
      try {
        ...
        if (isUrl(linkPost)) {
          let getInfo = await getPage(linkPost, ua, "buffer");
          // console.log(getInfo);
          if (getInfo == "err") {
            res.end("404");
          } else {
            ...
            if (typeContent) {
              if (typeContent.indexOf("text/html") == 0 && codeContent != 404) {
                ...
                let dataContent = getInfo.content;
                ...
                if (body == "err") {
                  ...
                  try {
                    let dom = new JSDOM(body, { looseMode: true, virtualConsole }).window
                      .document;
                ...
                if (codeContent == 404) {
                  ...
                  if (
                    ...
                    let datas = await getPage(linkPost, ua, "buffer");
                    res.set({ "content-type": typeContent });
                    ...
                    let dataXML = await getPage(linkFix, ua, "text");
                    ...
                    //--------- assets css host ---------
                    let dataCSS = await getPage(linkPost, ua, "buffer");
                    res.writeHead(200, resOriginHeader);
                    res.end(dataCSS.content);
                  } else {
                    res.end("404");
                  }
                }
